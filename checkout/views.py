...
from bookingkelas.models import Booking  # model revisi temanmu

# CART -> CHECKOUT PRODUK (alamat diinput tiap checkout)
@login_required
def cart_checkout_page(request):
    form = CartCheckoutForm()
    return render(request, "checkout/cart_checkout_page.html", {"form": form})
...
def checkout_cart_create(request):
    ...
    # Ambil cart user + preload product
    ...
    with transaction.atomic():
        # 1) Buat ProductOrder (snapshot alamat & kontak)
        order = ProductOrder.objects.create(
            user=request.user,
            cart=cart,
            receiver_name=cd["receiver_name"],
            receiver_phone=cd["receiver_phone"],
        ...
        # 2) Snapshot item cart -> ProductOrderItem
        for ci in cart.items.all():
            ProductOrderItem.objects.create(
                order=order,
                product=ci.product,                  # jejak (opsional)
                product_name=ci.product.name,        # snapshot
                unit_price=ci.product.price,         # snapshot
                quantity=ci.quantity,                # snapshot
            )

        # 3) Hitung total dan simpan
        order.recalc_totals()
        order.save(update_fields=["subtotal", "shipping_fee", "total"])

        # 4) Kosongkan cart HANYA jika commit sukses
        transaction.on_commit(lambda: cart.items.all().delete())

    messages.success(request, "Checkout sukses! Pembayaran: Cash on Delivery.")
    # redirect ke history (punyanya tim profil)
    return redirect("profile:order_history")
...
def checkout_booking_now(request, booking_id):
    """
    Checkout 1 booking saja.
    Dipanggil setelah user menekan 'Book Class' di modul booking.
    """
    if request.method != "POST":
        return HttpResponseBadRequest("POST required.")
